...
def download_ticker_data(ticker):
    ...
    try:
        ...
        url = f"https://eodhd.com/api/eod/{ticker}.US?period=d&api_token=***REMOVED_SECRET***&fmt=json"
...
def download_changes_ticker_data(args):
    ...
    try:
        ...
        if ticker in delisted_tickers.keys():
            ...
            response = requests.get(f"https://eodhd.com/api/eod/{old_ticker}.US?period=d&api_token=***REMOVED_SECRET***&fmt=json")
            ...
            response = requests.get(f"https://eodhd.com/api/eod/{ticker}.US?period=d&api_token=***REMOVED_SECRET***&fmt=json")
            if response.status_code == 200:
                data = response.json()
                return {ticker: pd.DataFrame(data)}
            else:
